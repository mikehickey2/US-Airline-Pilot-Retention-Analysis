...
>
> This validation document records findings from reviewing the original separate subfactor analysis.
> All recommendations have been implemented in the unified analysis: `scripts/analysis/unified_retention_analysis.qmd`.
> Referenced files are archived in `archive/original_qmds/` and `archive/old_scripts/`.

Generated from a static review of `scripts/analysis/subfactor_analysis.qmd` and `render_subfactor_analysis.R` (both now archived).
