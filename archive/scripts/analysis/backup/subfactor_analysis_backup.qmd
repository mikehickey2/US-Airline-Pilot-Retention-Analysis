...
#   - Experience: Quartiles contain exactly 19 pilots each
...
**Statistical Methods:** Mann-Whitney U tests were used for binary demographic comparisons, Kruskal-Wallis tests for experience quartiles, and Friedman tests for within-construct comparisons. All tests used proper respondent identifiers to maintain the repeated-measures structure of the data. Effect sizes (rank-biserial r for Mann-Whitney, eta-squared for Kruskal-Wallis) were computed to assess practical significance beyond statistical significance.

**Key Finding:** After conducting 155 demographic comparison tests with Bonferroni correction for multiple comparisons, **no significant demographic differences were detected** (p.adj < 0.05) in how pilots ranked subfactor priorities within any retention construct. Effect sizes for all comparisons were small (|r| < 0.3, η² < 0.06), suggesting minimal practical differences across demographic groups.
...
This analysis is based on n=76 valid pilot responses from regional airline pilots. Demographic distributions in the sample reflect the regional airline pilot population, with gender distribution (~11% female) consistent with industry norms (8-12% female representation at regional carriers). The age cutoff (≤35 vs >35) was selected based on the average age of first-year First Officers at legacy and major carriers. Experience quartiles contain exactly 19 pilots each.

All effect sizes were small in magnitude (|r| < 0.3, η² < 0.06), suggesting minimal practical differences across demographic groups.
