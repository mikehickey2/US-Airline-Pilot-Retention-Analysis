...
**Code Availability:** The complete analysis code is available at `scripts/analysis/unified_retention_analysis.qmd` in the [GitHub repository][repo-url].
...
dir.create(
  here("output/tables/unified_analysis"),
  showWarnings = FALSE,
  recursive = TRUE
)
dir.create(
  here("output/figures/unified_analysis"),
  showWarnings = FALSE,
  recursive = TRUE
)
```
...
#| fig-cap: "Distribution of top-ranked retention factors among U.S. airline pilots"
...
dir.create(
  here("output/tables/subfactor_analysis"),
  showWarnings = FALSE,
  recursive = TRUE
)
dir.create(
  here("output/figures/subfactor_analysis"),
  showWarnings = FALSE,
  recursive = TRUE
)
```
...
|------------|-------------|-------------------------------|
| Age groups | 58 vs 18 | Medium-large (r > 0.40) |
| Experience | ~19 per quartile | Large (eta² > 0.10) |
| Gender | 67 vs 8 | Very large only (r > 0.6) |
| Military | 69 vs 7 | Very large only (r > 0.6) |
| Position | 50 vs 26 | Medium (r ≈ 0.35) |
